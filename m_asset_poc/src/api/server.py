"""
FastAPI server for the M-Asset POC application.
Provides REST API endpoints for Text-to-SQL queries.
"""
...
from ..core.weaviate_pool import weaviate_pool
        ...
        async with concurrency_limiter.acquire() as request_info:
            ...
            if wait_time > 0:
                if not response.error_message:
                    response.error_message = f"요청이 {wait_time:.2f}초 대기 후 처리되었습니다."
            
            return response
            
    except asyncio.TimeoutError:
        logger.warning(f"Request timed out for query: {request.query}")
        return QueryResponse(
            success=False,
            query=request.query,
            generated_sql=None,
            result=None,
            error_message="요청 대기 시간이 초과되었습니다. 잠시 후 다시 시도해주세요.",
            session_id=request.session_id,
            user_id=request.user_id
        )
    except Exception as e:
...
async def get_cache_stats():
    ...
    """
    try:
        stats = llm_cache_manager.get_cache_stats()
        return {
            "success": True,
            "stats": stats
        }
...
async def clear_cache():
    ...
    """
    try:
        deleted_count = llm_cache_manager.clear_all_cache()
        logger.info(f"Cache cleared: {deleted_count} items deleted")
        return {
            "success": True,
            "deleted_count": deleted_count,
            "message": f"Successfully cleared {deleted_count} cached SQL queries"
        }
    except Exception as e:
...
@app.get("/vector-store/pool/stats")
async def get_vector_pool_stats():
    """
    벡터 스토어 연결 풀 통계 정보 조회
    ...
    try:
        ...
        logger.error(f"Error getting vector pool stats: {e}")
        raise HTTPException(status_code=500, detail=f"Error getting vector pool stats: {str(e)}")
