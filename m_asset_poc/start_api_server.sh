...
# API 서버 시작 스크립트
# 사용법: ./start_api_server.sh

echo "🚀 M-Asset API 서버를 시작합니다..."
...
# 기존 API 서버 프로세스 확인 및 종료
echo "🔍 기존 API 서버 프로세스를 확인합니다..."
API_PID=$(lsof -ti:8010 2>/dev/null)

if [ ! -z "$API_PID" ]; then
    echo "⚠️  포트 8010에서 실행 중인 프로세스를 발견했습니다 (PID: $API_PID)"
    echo "🛑 기존 프로세스를 종료합니다..."
...
if grep -q "LLM_CACHE_ENABLED=true" .env; then
    ...
    if command -v redis-cli &> /dev/null; then
        if redis-cli ping &> /dev/null; then
            ...
            echo "💡 Redis 시작: brew services start redis (macOS) 또는 sudo systemctl start redis (Linux)"
        fi
    fi
fi
...
echo "📝 로그는 api_server.log에 저장됩니다."
echo "⏹️  중지하려면 Ctrl+C를 누르세요."
echo "-" * 50

# 백그라운드로 실행하려면 아래 주석을 해제하세요
# nohup python main.py --mode server > api_server.log 2>&1 &
# echo "✅ API 서버가 백그라운드에서 시작되었습니다. (PID: $!)"
